...
const theme = {
  ...
  line: '#DAD9CC',
  lineLight: '#E8E7DC',
  accent: '#2C4A3B',       // deep forest
  accent2: '#B85A3E',      // muted terracotta
  accentSoft: '#E8EDE4',   // pale forest wash
  highlight: '#F3F1E6',
  danger: '#A84535',
  dangerBg: '#F0D8D2',
  warn: '#B8863E',
...
/* =========================================================
   ...
   ========================================================= */
const K = {
  members: 'sosun:members:v6',
  sessions: 'sosun:sessions:v6',
  classlog: 'sosun:classlog:v6',
  trials: 'sosun:trials:v6',
  smsLog: 'sosun:smsLog:v6',
  dashDismiss: 'sosun:dashDismiss:v6',
  smsConfirmed: 'sosun:smsConfirmed:v6',
  seeded: 'sosun:seeded:v6',
};
  ...
  // ==========================================================
  {
    ...
    passes: [
      ...
      },
      {
        ...
        totalSessions: 5, usedSessions: 0,
        paymentDate: '2026-04-21', startDate: '2026-04-21',
        expiryDate: computeExpiry('2026-04-21', 70),
        price: 350000, pricePerSession: 70000, sessionDates: [],
        note: '스타터 패키지 A에서 전환 · 정가 80,000원 → 할인가 70,000원',
...
const SEED_TRIALS = [
  ...
  { name: '김혜정', phone: '010-3143-3536', date: '2026-04-28', time: '19:20', experience: '요가 경험 무', painPoints: '해당없음', status: '수업완료', memo: '4/23 19:20 요청 → 4/28 19:20으로 변경 요청', source: '카채널', paid: true },
  { name: '고하은', phone: '010-6888-4056', date: '2026-04-28', time: '19:20', experience: '요가 경험 유', painPoints: '해당없음', status: '수업완료', memo: '4/23 19:20 요청 → 4/28 19:20으로 변경 요청', source: '카채널', paid: true },
  { name: '김정락', phone: '010-9182-2222', date: '2026-04-28', time: '19:20', experience: '요가 4~5번', painPoints: '허리, 등', status: '수업완료', memo: '', source: '카채널', paid: true },
  { name: '송유진', phone: '010-7484-0502', date: '2026-04-28', time: '19:20', experience: '요가 경험 없음', painPoints: '불편한 부위 없음', status: '수업완료', memo: '', source: '인스타', paid: true },
...
function passStatus(p) {
  ...
  let tone, label;
  if (p.archived && p.convertedTo) { tone = 'neutral'; label = '전환됨'; }
  else if (done) { tone = 'success'; label = '완료'; }
  else if (p.expiryDate < today) { tone = 'danger'; label = '만료'; }
  else if (daysLeft <= 7) { tone = 'warn'; label = `D-${daysLeft}`; }
  else { tone = 'neutral'; label = `D-${daysLeft}`; }
  return { tone, label, daysLeft, done };
}
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4"
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: theme.line }}>
          <div className="font-bold text-[15px]" style={{ color: theme.ink, fontFamily: 'Noto Sans KR, sans-serif' }}>{title}</div>
      ...
      <div className="px-4 pt-3 pb-2 flex items-center justify-between">
        <div className="flex items-baseline gap-3">
          <div style={{
            ...
            fontSize: 26, color: theme.ink,
            letterSpacing: '-0.02em', lineHeight: 1,
          }}>
            소선요가
          </div>
          <div className="text-[12px] flex items-baseline gap-1.5"
            style={{ color: theme.inkMute, fontFamily: 'Cormorant Garamond, serif' }}>
            <span style={{ color: theme.accent, fontStyle: 'italic', fontWeight: 500 }}>素禪</span>
            <span>·</span>
            <span style={{ fontStyle: 'italic' }}>So-Seon</span>
          </div>
        </div>
  ...
  return (
    <Modal open={open} onClose={onClose} title={template.title} maxWidth="max-w-md">
      <div className="space-y-3">
        ...
        {step === 'compose' && (
          <>
            <div className="p-3 rounded-xl text-[13px] whitespace-pre-wrap"
        ...
        {step === 'confirm' && (
          <>
            <div className="p-3 rounded-xl text-center" style={{ backgroundColor: theme.highlight }}>
...
/* =========================================================
   ...
   ========================================================= */
function HomeView({ members, sessions, trials, dashDismiss, setDashDismiss, smsConfirmed = {}, toast, onSendSMS, goto, onOpenSettings }) {
      ...
      (m.passes || []).forEach(p => {
        ...
        if (daysUntilStart <= 7 && daysUntilStart >= 0) {
          list.push({
            ...
            priority: 2,
            member: m, pass: p,
            title: `${m.name} · ${p.type} 시작 D-${daysUntilStart}`,
  ...
  return (
    <div className="px-3 pb-28 pt-2 space-y-4">
      ...
      <div className="rounded-2xl p-4" style={{ backgroundColor: theme.card, border: `1px solid ${theme.line}` }}>
        ...
        <div className="text-lg font-bold mt-0.5" style={{ color: theme.ink }}>
          오늘도 잘 이끌어 주세요 🌿
        </div>
        <div className="grid grid-cols-3 gap-2 mt-3">
          <Stat label="오늘 수업" value={todaySessions.length} color={theme.accent} />
          <Stat label="활성 회원" value={activeMembers} color={theme.accent} />
          <Stat label="만료 임박" value={expiringSoon} color={theme.accent2} />
        </div>
      </div>
      ...
      {daysSinceBackup !== null && daysSinceBackup >= 7 && (
        <div className="rounded-2xl p-3 flex items-start gap-2"
          style={{ backgroundColor: theme.warnBg, border: `1px solid #C8A366` }}>
          ...
          <div className="flex-1">
            ...
            <div className="text-[11px] mt-0.5" style={{ color: theme.inkSoft }}>
              {daysSinceBackup === 999
                ? '아직 백업한 적이 없어요. 설정에서 백업 파일을 내보내세요.'
                : `마지막 백업 후 ${daysSinceBackup}일이 지났어요. 주 1회 백업을 추천해요.`}
            </div>
          </div>
          <button onClick={onOpenSettings}
      ...
      <div>
        <div className="flex items-center justify-between mb-2 px-1">
          <div className="text-sm font-semibold" style={{ color: theme.ink }}>
            <Calendar size={14} className="inline mr-1" style={{ color: theme.accent }} /> 오늘 수업
          </div>
        ...
        {todaySessions.length === 0 ? (
          <div className="rounded-2xl p-4 text-center text-[13px]"
            style={{ backgroundColor: theme.card, border: `1px solid ${theme.lineLight}`, color: theme.inkMute }}>
            오늘 예정된 수업이 없어요
          </div>
        ) : (
          <div className="space-y-1.5">
            {todaySessions.map(s => {
              ...
              return (
                <div key={s.date + s.time} className="rounded-xl p-3"
                  style={{ backgroundColor: theme.card, border: `1px solid ${theme.lineLight}` }}>
                  <div className="flex items-baseline gap-2">
                    <span className="text-base font-bold tabular-nums" style={{ color: theme.accent, fontFamily: 'Cormorant Garamond, serif' }}>
                      {s.time}
                    </span>
                    {s.note && <Chip tone="terra" size="sm">{s.note}</Chip>}
                  </div>
                  <div className="mt-1 flex flex-wrap gap-x-2 gap-y-0.5">
                    {active.map((p, i) => (
                      <span key={i} className="text-[13px]" style={{ color: theme.ink }}>
                        {p.memberName}
                    ...
                    {cancelled.map((p, i) => (
                      <span key={'c' + i} className="text-[12px] line-through"
                        style={{ color: p.cancelled === 'charged' ? theme.danger : theme.inkMute }}>
                        {p.memberName}
                      </span>
                    ))}
                  </div>
...
function Stat({ label, value, color }) {
  return (
    <div className="text-center py-2 rounded-xl" style={{ backgroundColor: theme.cardAlt2 }}>
      <div className="text-2xl font-bold tabular-nums" style={{ color, fontFamily: 'Cormorant Garamond, serif' }}>{value}</div>
...
/* =========================================================
   Schedule View
   ========================================================= */
function ScheduleView({ members, setMembers, sessions, setSessions, toast }) {
  ...
  const getSlotView = (date, time) => {
    ...
    if (explicit) return { participants: explicit.participants, isAuto: false, note: explicit.note };
  ...
  const handleTouchEnd = (e) => {
    ...
    // Horizontal swipe only (dx > 50, dy small)
    ...
    <div className="px-3 pb-28 pt-2" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="text-center">
          <div className="text-[13px]" style={{ color: theme.inkMute }}>{weekStart.getFullYear()}년</div>
      ...
      <div className="flex justify-between items-center mb-2">
        ...
        <div className="flex items-center gap-2 text-[10px]" style={{ color: theme.inkMute }}>
          <span className="flex items-center gap-1">
            <span className="w-2.5 h-2.5 rounded-sm" style={{ backgroundColor: theme.accentSoft, border: `1px solid ${theme.accent}` }}></span>
            소그룹 (화·목)
          </span>
        </div>
      </div>

      <div className="rounded-2xl overflow-hidden shadow-sm" style={{ backgroundColor: theme.card, border: `1px solid ${theme.line}` }}>
        <div className="overflow-x-auto">
          <table className="w-full text-[12px]" style={{ minWidth: 600 }}>
            <thead>
              <tr>
                <th className="py-2 px-2 text-left font-medium w-14"
                  style={{ color: theme.inkSoft, borderBottom: `1px solid ${theme.line}`, backgroundColor: theme.cardAlt }}>
                  시간
                </th>
                {days.map((d, i) => (
                  <th key={i} className="py-2 px-1 font-medium"
                    style={{
                      color: theme.ink,
                      borderBottom: `1px solid ${theme.line}`,
                      borderLeft: `1px solid ${theme.line}`,
                      backgroundColor: smallGroupDay(d) ? theme.accentSoft : theme.cardAlt,
                    }}>
                    <div style={{ fontWeight: 700 }}>{WEEK_KR[d.getDay()]}</div>
                    <div className="text-[11px] font-normal" style={{ color: theme.inkSoft }}>
            ...
            <tbody>
              {TIME_PRESETS.map((time) => (
                <tr key={time}>
                  <td className="py-2 px-2 font-bold tabular-nums align-top"
                    style={{ color: theme.accent2, backgroundColor: theme.cardAlt + '88', borderBottom: `1px solid ${theme.line}`, fontFamily: 'Cormorant Garamond, serif', fontSize: 13 }}>
                    {time}
                  </td>
                  {days.map((d, i) => {
                    const view = getSlotView(d, time);
                    const isToday = toYMD(d) === todayYMD;
                    const smDay = smallGroupDay(d);
                    return (
                      <td key={i}
                        onClick={() => setSlotModal({ date: d, time, existing: sessions[`${toYMD(d)}_${time}`], autoFill: view?.isAuto ? view.participants : null })}
                        className="align-top p-1 cursor-pointer transition-colors"
                        style={{
                          borderBottom: `1px solid ${theme.line}`,
                          borderLeft: `1px solid ${theme.line}`,
                          backgroundColor: isToday ? theme.highlight : smDay ? theme.accentSoft + '55' : 'transparent',
                          minWidth: 106, height: 66,
                        }}>
                        {view ? (
                          <div className="space-y-0.5">
                            {view.note && (
                              <div className="text-[9px] font-semibold uppercase tracking-wide" style={{ color: theme.accent2 }}>
                                {view.note}
                              </div>
                            )}
                            {view.participants.map((p, j) => {
                              ...
                              return (
                                <div key={j} className="text-[11px] leading-tight"
                                  style={{
                                    ...
                                    opacity: view.isAuto ? 0.55 : 1,
                                    fontStyle: view.isAuto ? 'italic' : 'normal',
                                    textDecoration: isCancelled ? 'line-through' : 'none',
                                  }}>
                                  <span style={{ fontWeight: p.isTrial ? 400 : 500 }}>{p.memberName}</span>
                                  {p.sessionNumber && p.totalSessions && !isCancelled && (
                                    <span style={{ color: theme.inkMute }}> ({p.sessionNumber}/{p.totalSessions})</span>
                                  )}
                                  {p.isTrial && <span style={{ color: theme.accent2, marginLeft: 2 }}>·체험</span>}
                                  {p.classType === '개인' && <span style={{ color: theme.accent, marginLeft: 2 }}>·개인</span>}
                                </div>
                              );
                            })}
                          </div>
                          ...
                          <div className="h-full flex items-center justify-center opacity-0 hover:opacity-50">
                            <Plus size={12} style={{ color: theme.inkMute }} />
                          </div>
                        )}
                      </td>
                    );
      ...
      <div className="mt-2 text-[11px] text-center" style={{ color: theme.inkMute }}>
        <span style={{ fontStyle: 'italic', opacity: 0.7 }}>흐린 글씨</span>는 고정 슬롯 자동 표시 · 탭해서 확정하세요
      </div>

      {slotModal && (
        <SessionEditor
          slot={slotModal} members={members}
          onClose={() => setSlotModal(null)}
          onSave={async (data) => {
            await saveSession(slotModal.date, slotModal.time, data);
            setSlotModal(null);
            toast('저장되었어요');
          }}
        />
...
function SessionEditor({ slot, members, onClose, onSave }) {
  ...
  const addExisting = () => {
    ...
    if (!m) return;
    const pass = activePass(m, 'group') || activePass(m, 'private');
    setParts([...parts, {
      memberId: m.id, memberName: m.name, passId: pass?.id,
      sessionNumber: pass ? pass.usedSessions + 1 : undefined,
      totalSessions: pass?.totalSessions,
      classType: pass?.category === 'private' ? '개인' : undefined,
    }]);
    setAddingMember('');
  };
  ...
  return (
    <Modal open={true} onClose={onClose} title={`${fmtKR(slot.date)} · ${slot.time}`} maxWidth="max-w-md">
      <div className="space-y-4">
        ...
        <div className="flex justify-between gap-2 pt-2">
          {existing && (
            <Button variant="danger" size="sm" icon={Trash2} onClick={() => onSave({ participants: [] })}>수업 삭제</Button>
          )}
          <div className="flex gap-2 ml-auto">
            <Button variant="ghost" onClick={onClose}>취소</Button>
            <Button icon={Check} onClick={() => onSave({ participants: parts, note })}>저장</Button>
          </div>
        </div>
      </div>
    </Modal>
...
function MembersView({ members, setMembers, sessions, toast, onSendSMS }) {
  ...
  const addManyMembers = async (rows) => {
    const today = toYMD(new Date());
    ...
    const buildPasses = (m) => {
      const pass = m.pass || {};
      const typeText = `${pass.type || m.passType || ''} ${m.notes || ''}`;
      const paymentDate = pass.paymentDate || m.paymentDate || today;
      const startDate = pass.startDate || m.startDate || paymentDate || today;
      const price = Number(pass.price ?? m.price ?? 0);

      if (/스타터|starter/i.test(typeText)) {
        return [
          {
            id: uid(), type: '스타터 · 개인레슨 3회', category: 'private',
            totalSessions: 3, usedSessions: Number(pass.privateUsed || 0), sessionDates: [],
            days: 90, paymentDate, startDate, expiryDate: computeExpiry(startDate, 90),
            price: 0, bundleOf: '스타터 패키지', note: pass.note || m.passMemo || undefined,
          },
          {
            id: uid(), type: '스타터 · 소그룹 6회', category: 'group',
            totalSessions: 6, usedSessions: Number(pass.groupUsed || 0), sessionDates: [],
            days: 35, paymentDate, startDate, expiryDate: pass.expiryDate || computeExpiry(startDate, 35),
            price: price || 360000, bundleOf: '스타터 패키지', note: pass.note || m.passMemo || undefined,
          },
        ];
      }

      const totalSessions = Number(pass.totalSessions || m.totalSessions || 0);
      if (!totalSessions) return [];
      const days = Number(pass.days || m.days || 42);
      return [{
        id: uid(),
        type: pass.type || m.passType || `${totalSessions}회 수강권`,
        category: pass.category || m.category || 'group',
        totalSessions, usedSessions: Number(pass.usedSessions || 0), sessionDates: [],
        days, paymentDate, startDate, expiryDate: pass.expiryDate || computeExpiry(startDate, days),
        price, note: pass.note || m.passMemo || undefined,
      }];
    };
    ...
    const newMembers = rows
      ...
      .map(m => ({
        ...
        memoTimeline: m.memo ? [{ id: uid(), date: today, text: m.memo }] : [],
        progressLog: [],
  ...
  const filtered = members.filter(m => {
    ...
    if (filter === 'private') return cat === 'private' || cat === 'both';
    if (filter === 'group') return cat === 'group' || cat === 'both';
    return true;
  });
    ...
    <div className="px-3 pb-28 pt-2">
      <div className="flex items-center justify-between mb-3">
        <div className="text-[13px]" style={{ color: theme.inkSoft }}>
          전체 <span className="font-bold" style={{ color: theme.ink }}>{members.length}명</span>
        </div>
        <div className="flex gap-1.5">
          <Button icon={Camera} variant="soft" size="sm" onClick={() => setBulkImporting(true)}>사진</Button>
          <Button icon={Plus} onClick={() => setAdding(true)}>회원 추가</Button>
        </div>
      </div>
      ...
      {members.length > 0 && (
        <div className="flex gap-1 mb-3">
          {[
            ...
            { id: 'group', label: '소그룹', count: counts.group },
            { id: 'private', label: '개인', count: counts.private },
          ].map(f => (
...
function MemberBulkImport({ onClose, onSave, toast }) {
  ...
  "members": [
    {
      ...
      "pass": {
        ...
        "category": "starter|group|private|trial",
        "totalSessions": 0,
        "usedSessions": 0,
...
날짜 규칙:
- 사진 속 4/23, 4.23 같은 날짜는 2026년으로 보정하세요.
- 스타터 패키지는 category를 starter로 쓰고 price를 결제금액으로 넣으세요.
- 회원권 신청서의 시작일/결제일/결제금액을 우선 사용하세요.
- 요일/시간이 화목 19:20처럼 보이면 fixedSlots에 화=2, 목=4로 넣으세요.
- 영수증에 스타터 패키지 A 360,000원이 보이면 pass.type="스타터 패키지 A", category="starter", price=360000으로 넣으세요.`;
      ...
      if (result?.members?.length) {
        ...
        toast('읽을 수 없었어요. 사진을 더 밝게 찍어서 다시 시도해주세요');
      }
    } catch (e) {
    ...
    <Modal open={true} onClose={onClose} title="사진으로 회원 등록" maxWidth="max-w-lg">
      <div className="space-y-3">
        <div className="text-[12px] leading-relaxed" style={{ color: theme.inkSoft }}>
          회원권 신청서, 상태 기록지, 결제 영수증, 상담 메모 사진을 올리면 회원 정보와 수강권을 자동으로 읽어옵니다.
        </div>
        ...
        <button onClick={() => fileRef.current?.click()}
          ...
          <div className="text-[11px]" style={{ color: theme.inkMute }}>여러 장 한 번에 가능</div>
        </button>
        ...
        <Button icon={Sparkles} onClick={analyze} disabled={loading || !images.length} className="w-full">
          {loading ? <><Loader2 size={14} className="animate-spin" /> 사진 읽는 중...</> : 'AI로 회원 정보 읽기'}
        </Button>
        ...
        {parsed && (
          <div className="space-y-2">
            ...
            {parsed.map((m, i) => (
              <div key={i} className="rounded-xl p-3 text-[12px] space-y-1" style={{ backgroundColor: theme.card, border: `1px solid ${theme.line}` }}>
                <div className="font-bold text-sm" style={{ color: theme.ink }}>{m.name || '이름 확인 필요'}</div>
                <div style={{ color: theme.inkSoft }}>{m.phone || '연락처 없음'} · {m.gender || '성별 미확인'} · {m.birthYear || '출생년도 미확인'}</div>
                {(m.keyPoint || m.painPoints) && <div style={{ color: theme.accent2 }}>⚠ {m.keyPoint || m.painPoints}</div>}
                {m.pass && <div style={{ color: theme.accent }}>수강권: {m.pass.type || m.pass.category} · {Number(m.pass.price || 0).toLocaleString()}원</div>}
                {m.notes && <div className="whitespace-pre-wrap" style={{ color: theme.inkMute }}>{m.notes}</div>}
              </div>
            ))}
    ...
    <div onClick={onClick} className="rounded-2xl p-3.5 cursor-pointer transition-all active:scale-[0.99]"
      ...
      <div className="flex items-start justify-between mb-2">
        <div>
          ...
          {member.keyPoint && (
            <div className="text-[10px] mt-0.5 truncate" style={{ color: theme.accent2, maxWidth: 240 }}>
              ⚠ {member.keyPoint}
            </div>
          )}
        </div>
        <div className="flex flex-col items-end gap-1">
          {ps && <Chip tone={ps.tone}>{ps.label}</Chip>}
        ...
        <div>
          <div className="flex justify-between text-[12px] mb-1">
            ...
            <span className="font-medium tabular-nums" style={{ color: theme.ink }}>
              {pass.usedSessions}/{pass.totalSessions}회
            </span>
          </div>
          <div className="h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: theme.cardAlt }}>
            <div className="h-full rounded-full transition-all" style={{ width: `${progress * 100}%`, backgroundColor: theme.accent }} />
          </div>
          <div className="text-[10px] mt-1 flex justify-between" style={{ color: theme.inkMute }}>
    ...
    <Modal open={true} onClose={onClose} title={member ? '회원 정보 수정' : '새 회원'}>
      <div className="space-y-3">
        ...
        <div>
          ...
          <div className="rounded-xl p-2" style={{ backgroundColor: theme.cardAlt, border: `1px solid ${theme.line}` }}>
  ...
  return (
    <Modal open={true} onClose={onClose} title={member.name} maxWidth="max-w-lg">
      <div className="space-y-4">
        {/* Key point banner */}
        {member.keyPoint && (
          <div className="p-3 rounded-xl flex gap-2 items-start" style={{ backgroundColor: theme.warnBg, border: `1px solid #C8A366` }}>
            <AlertCircle size={16} className="mt-0.5 shrink-0" style={{ color: theme.warn }} />
            <div>
        ...
        {tab === 'passes' && (
          <div className="space-y-3">
            ...
            {(() => {
              ...
              return (
                <div className="rounded-xl p-3" style={{ backgroundColor: theme.accentSoft, border: `1px solid ${theme.accent}44` }}>
                  {member.keyPoint && (
            ...
            {(member.passes || []).filter(p => !p.archived).map(p => {
              ...
              return (
                <div key={p.id} className="rounded-xl p-3" style={{ backgroundColor: theme.cardAlt, border: `1px solid ${theme.line}` }}>
                  <div className="flex items-start justify-between mb-2">
                    <div>
                    ...
                    <div className="flex flex-col items-end gap-1">
                      {st && <Chip tone={st.tone} size="sm">{st.label}</Chip>}
                    </div>
                  </div>
                  <div className="flex justify-between text-[12px] mb-1">
            ...
            {(member.passes || []).filter(p => p.archived).length > 0 && (
              <details className="rounded-xl p-3" style={{ backgroundColor: theme.cardAlt2, border: `1px dashed ${theme.line}` }}>
                ...
                {history.map((h, i) => {
                  ...
                  return (
                    <div key={i} className="px-3 py-2 rounded-lg flex items-start gap-3"
                      ...
                      <span className="text-[12px] font-medium tabular-nums" style={{ color: isCharged ? theme.danger : theme.accent2, fontFamily: 'Cormorant Garamond, serif', fontSize: 13 }}>{h.time}</span>
...
function MemoTimeline({ items, onUpdate, toast }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {adding && (
        <div className="rounded-xl p-3 space-y-2" style={{ backgroundColor: theme.cardAlt, border: `1px solid ${theme.line}` }}>
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-2">
          {items.map(m => (
            <div key={m.id} className="rounded-xl p-3" style={{ backgroundColor: theme.card, border: `1px solid ${theme.lineLight}` }}>
...
JSON만 출력. 코드펜스 금지.`;
  ...
  return (
    <div className="space-y-2">
      ...
      {adding && (
        <div className="rounded-xl p-3 space-y-3" style={{ backgroundColor: theme.cardAlt, border: `1px solid ${theme.line}` }}>
          <div className="flex gap-1 text-[12px]">
          ...
          {mode === 'text' ? (
            ...
            <>
              <input ref={fileRef} type="file" accept="image/*" multiple className="hidden" onChange={(e) => handleFiles(e.target.files)} />
              <button onClick={() => fileRef.current?.click()}
                className="w-full py-6 rounded-xl flex flex-col items-center gap-1.5 border-2 border-dashed"
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-2">
          {items.map(p => (
            <div key={p.id} className="rounded-xl p-3" style={{ backgroundColor: theme.card, border: `1px solid ${theme.lineLight}` }}>
...
function AssessmentPanel({ member, onUpdate, toast }) {
  ...
  if (!a) {
    return (
      <div className="text-center py-6">
        ...
        <div className="text-xs mt-1 mb-3" style={{ color: theme.inkMute }}>
          "AI 분석" 탭에서 사진이나 메모를 업로드해 주세요
        </div>
      </div>
    );
  }
  return (
    <div className="space-y-3 text-sm">
      {a.keyPoint && (
        <div className="p-3 rounded-xl" style={{ backgroundColor: theme.highlight }}>
          <div className="text-[11px] font-semibold mb-1" style={{ color: theme.accent2 }}>현재 몸의 핵심 포인트</div>
      ...
      {a.observations?.length > 0 && <Section title="해부학적 관찰" items={a.observations} color={theme.accent} />}
      {a.recommendations?.length > 0 && <Section title="수업에 넣으면 좋을 동작" items={a.recommendations} color={theme.accent2} bullet="▸" />}
      {a.cautions?.length > 0 && <Section title="주의할 점" items={a.cautions} color={theme.danger} bullet="⚠" />}
      {a.sourceImages?.length > 0 && (
        <details>
          <summary className="text-[11px] cursor-pointer" style={{ color: theme.inkMute }}>원본 이미지 {a.sourceImages.length}장</summary>
          <div className="mt-2 flex flex-wrap gap-2">
            {a.sourceImages.map((img, i) => (
              <img key={i} src={`data:${img.media_type};base64,${img.data}`}
                className="w-24 h-24 object-cover rounded-lg cursor-pointer"
                onClick={() => window.open(`data:${img.media_type};base64,${img.data}`, '_blank')} alt="" />
            ))}
          </div>
        </details>
      )}
      {a.rawInput && (
        <details>
          <summary className="text-[11px] cursor-pointer" style={{ color: theme.inkMute }}>원본 입력 텍스트</summary>
          <div className="mt-1 p-2 rounded-lg text-[12px] whitespace-pre-wrap" style={{ backgroundColor: theme.cardAlt, color: theme.inkSoft }}>
            {a.rawInput}
          </div>
        </details>
      )}
      <div className="text-[10px] text-right" style={{ color: theme.inkMute }}>업데이트 {a.updatedAt}</div>
...
function PassConvertDialog({ oldPass, onClose, onConvert }) {
  ...
  return (
    <Modal open={true} onClose={onClose} title="수강권 전환">
      <div className="space-y-3">
        <div className="p-3 rounded-xl" style={{ backgroundColor: theme.cardAlt }}>
              ...
              <div key={e.id} className="p-2.5 rounded-lg"
                ...
                <div className="flex items-start gap-2">
                  <div className="text-[12px] font-bold tabular-nums min-w-[46px]"
                    style={{ color: isCancelledEntry ? theme.inkMute : theme.accent, fontFamily: 'Cormorant Garamond, serif', fontSize: 14 }}>
                    {e.time}
                  </div>
...
function TrialsView({ trials, setTrials, members, setMembers, toast, onSendSMS }) {
  ...
  const statusTone = (s) => {
    return s === '회원전환' ? 'success' : s === '미전환' ? 'neutral' : s === '수업완료' ? 'accent' : 'warn';
  };
  ...
  const filteredTrials = trials.filter(t => {
    if (filter === 'converted') return t.status === '회원전환';
    if (filter === 'not_converted') return t.status !== '회원전환';
    return true;
  });
            ...
            <div key={t.id} onClick={() => setEditing(t)}
              ...
              <div className="flex items-start justify-between mb-1">
                <div>
                  ...
                  {t.phone && <div className="text-[11px]" style={{ color: theme.inkMute }}>{t.phone}</div>}
                </div>
                <Chip tone={statusTone(t.status)} size="sm">{t.status}</Chip>
              </div>
              {t.date && (
                <div className="text-[12px]" style={{ color: theme.accent }}>
                  <Calendar size={11} className="inline mr-0.5" /> {t.date} {t.time}
                </div>
              )}
              {t.painPoints && (
                <div className="text-[11px] mt-1 truncate" style={{ color: theme.accent2 }}>
                  ⚠ {t.painPoints}
                </div>
              )}
              {t.experience && (
                <div className="text-[11px] mt-0.5" style={{ color: theme.inkSoft }}>경험: {t.experience}</div>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="space-y-3">
        {!parsed && (
          <>
            ...
            <input ref={fileRef} type="file" accept="image/*" multiple className="hidden" onChange={(e) => handleFiles(e.target.files)} />
            <button onClick={() => fileRef.current?.click()}
              className="w-full py-8 rounded-xl flex flex-col items-center gap-1.5 border-2 border-dashed"
        ...
        {parsed && (
          <>
            ...
            <div className="space-y-2 max-h-[50vh] overflow-y-auto">
              {parsed.map((t, i) => (
                <div key={i} className="rounded-xl p-3" style={{ backgroundColor: theme.cardAlt, border: `1px solid ${theme.lineLight}` }}>
  ...
  return (
    <div className="px-3 pb-28 pt-2 space-y-4">
      ...
      <div className="rounded-2xl p-4" style={{ backgroundColor: theme.card, border: `1px solid ${theme.line}` }}>
        ...
        <div className="flex items-baseline gap-2">
          <div className="text-3xl font-bold tabular-nums" style={{ color: theme.accent, fontFamily: 'Cormorant Garamond, serif' }}>
      ...
      <div className="rounded-2xl p-4" style={{ backgroundColor: theme.card, border: `1px solid ${theme.line}` }}>
        ...
        <div className="flex items-baseline gap-2">
          <div className="text-3xl font-bold tabular-nums" style={{ color: theme.accent2, fontFamily: 'Cormorant Garamond, serif' }}>
...
/* =========================================================
   ...
   ========================================================= */
const K_LAST_BACKUP = 'sosun:lastBackup:v6';

function SettingsModal({ open, onClose, members, sessions, classLog, trials, setMembers, setSessions, setClassLog, setTrials, toast }) {
  const fileRef = useRef();
  ...
  return (
    <Modal open={open} onClose={onClose} title="설정" maxWidth="max-w-md">
      <div className="space-y-4">
        {/* Backup section */}
        <div className="rounded-xl p-3" style={{ backgroundColor: theme.cardAlt }}>
        ...
        {/* Stats */}
        <div className="rounded-xl p-3" style={{ backgroundColor: theme.card, border: `1px solid ${theme.line}` }}>
...
export default function App() {
  ...
  const [smsDialog, setSmsDialog] = useState(null); // { phone, name, template, alertId }
  const [settingsOpen, setSettingsOpen] = useState(false);
  const [smsConfirmed, setSmsConfirmed] = useState({}); // { alertId: timestamp }
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!seeded) {
        ...
        s = buildSeedSessions();
        c = SEED_CLASSLOG;
        t = SEED_TRIALS.map(tr => ({ id: uid(), ...tr, createdAt: tr.date || toYMD(new Date()) }));
        dd = {};
      ...
      setMembers(m); setSessions(s); setClassLog(c); setTrials(t); setDashDismiss(dd);
      setSmsConfirmed(sc);
      setReady(true);
    })();
  }, []);
  ...
  if (!ready) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: theme.bg }}>
        <div className="text-center">
          <div className="text-2xl font-bold mb-2" style={{ fontFamily: '"Black Han Sans", "Noto Sans KR", sans-serif', color: theme.ink }}>
            소선요가
          </div>
          <div className="text-xs" style={{ color: theme.accent, fontFamily: 'Cormorant Garamond, serif', fontStyle: 'italic' }}>
            素禪 · So-Seon
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: theme.bg, fontFamily: 'Noto Sans KR, sans-serif', color: theme.ink }}>
      <style>{`
      ...
      {tab === 'schedule' && (
        <ScheduleView members={members} setMembers={setMembers}
          sessions={sessions} setSessions={setSessions} toast={toast} />
      )}
      {tab === 'members' && (
      ...
      {tab === 'analysis' && (
        <AnalysisView members={members} setMembers={setMembers} toast={toast} />
      )}
      {tab === 'stats' && (
