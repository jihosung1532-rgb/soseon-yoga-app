...
const SEED_MEMBERS = [
  ...
  // 10. 조상범 (테스트 계정 - 사장님 남친)
  {
    id: 'm_chosangbeom', name: '조상범', phone: '010-7701-2202', birthYear: '',
    gender: '남', createdAt: '2026-05-02',
    fixedSlots: [
      { dow: 2, time: '19:20' },
      { dow: 4, time: '19:20' },
    ],
    keyPoint: '코어 강화 필요',
    notes: '테스트 계정 · 코어 강화 필요',
    progressLog: [],
    memoTimeline: [
      { id: 'mm_csb1', date: '2026-05-02', text: '체험 후 당일 정식 등록. 1개월권 8회 (200,000원, 계좌이체). 화·목 19:20 고정. 코어 강화 우선.' },
    ],
    analysis: {
      summary: '체험 후 정식 등록한 회원으로 코어 강화가 가장 큰 과제입니다. 플랭크 변형부터 시작해 점진적으로 난이도를 올리며 코어 안정성 훈련에 집중합니다.',
      keyPoint: '코어 약함 · 코어 안정성 강화 우선',
      observations: [
        '코어 안정성 약함',
        '체력 보통',
        '요가 초보자',
      ...
      rawInput: '체험 후 등록 2026-05-02',
      updatedAt: '2026-05-02',
    },
    passes: [{
      id: 'p_csb1', type: '1개월 (8회)', category: 'group',
      totalSessions: 8, usedSessions: 0,
      paymentDate: '2026-05-02', startDate: '2026-05-02',
      expiryDate: computeExpiry('2026-05-02', 42),
      price: 200000, paymentMethod: '계좌이체',
      sessionDates: [],
      note: '체험 후 당일 정식 등록. 화·목 19:20 고정.',
    }],
  },
];
...
const SEED_TRIALS = [
  ...
  { name: '조상범', phone: '010-7701-2202', date: '2026-05-02', time: '19:20', experience: '', painPoints: '코어 약함', status: '회원전환', convertedAt: '2026-05-02', convertedToMemberId: 'm_chosangbeom', memo: '체험 후 당일 정식 등록 (테스트 계정)', source: '소개', paid: true },
...
function CapacitySheet({ time, allTimes, capacityStatus, capacity, onClose, onChangeTime }) {
    ...
    return (
      <div key={m.id} className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl"
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[10px] mt-0.5" style={{ color: theme.inkMute, fontFamily: theme.serif, fontStyle: 'italic' }}>
            since {since.replace(/-/g, '.')}
          </div>
        </div>
        {isUpcoming ? (
          <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold"
            style={{ backgroundColor: 'rgba(212,181,116,0.18)', color: '#8A6418' }}>
            시작 예정
          </span>
        ) : (
          <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold"
            style={{ 
              backgroundColor: remaining <= 2 ? 'rgba(184,84,80,0.10)' : 'rgba(44,74,59,0.08)', 
              color: remaining <= 2 ? '#B85450' : theme.accent,
            }}>
            {remaining}회 남음
          </span>
        )}
  ...
  // 빈 슬롯 placeholder
  const emptySlot = (count) => Array(Math.max(0, capacity - count)).fill(0).map((_, i) => (
    <div key={`empty-${i}`} className="flex items-center justify-center px-3 py-2.5 rounded-xl"
      style={{ 
  ...
  return (
    <div onClick={onClose} 
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        {/* 핸들 */}
        <div className="mx-auto mb-3" style={{ width: 36, height: 4, background: '#C8C7BA', borderRadius: 2 }}></div>
        
        {/* 타이틀 */}
        <div className="flex items-baseline justify-between mb-3">
          <div style={{ fontFamily: theme.serif, fontSize: 22, fontWeight: 500, color: theme.ink }}>
            고정 회원
          </div>
          <div className="text-[11px]" style={{ color: theme.inkSoft }}>
            화 <strong style={{ color: theme.ink }}>{data.tueCount}</strong>명
            <span className="mx-1.5" style={{ color: theme.inkMute }}>·</span>
            목 <strong style={{ color: theme.ink }}>{data.thuCount}</strong>명
          </div>
        </div>
        ...
        {allTimes.length > 1 && (
          <div className="flex gap-1 p-1 rounded-xl mb-3"
            style={{ backgroundColor: theme.cardAlt2 || '#F5F4EC' }}>
            {allTimes.map(t => {
              const active = t === time;
              return (
                <button key={t}
        ...
        <div className="mb-3.5">
          <div className="text-[10px] font-bold mb-1.5 px-1" 
            style={{ color: theme.inkMute, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            화요일 · {data.tueCount}/{capacity}
          </div>
          <div className="flex flex-col gap-1.5">
            {data.tueMembers.map(renderRow)}
            {emptySlot(data.tueCount)}
          </div>
        </div>
        ...
        <div className="mb-3">
          <div className="text-[10px] font-bold mb-1.5 px-1" 
            style={{ color: theme.inkMute, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            목요일 · {data.thuCount}/{capacity}
          </div>
          <div className="flex flex-col gap-1.5">
            {data.thuMembers.map(renderRow)}
            {emptySlot(data.thuCount)}
          </div>
        </div>
        
        {/* 닫기 */}
        <button onClick={onClose}
          className="w-full py-3 rounded-xl text-[13px] font-medium mt-2"
          style={{ backgroundColor: 'transparent', border: `1px solid ${theme.line}`, color: theme.inkSoft }}>
          닫기
        </button>
      </div>
    </div>
...
function HomeView({ members, setMembers, sessions, setSessions, trials, classLog, dashDismiss, setDashDismiss, smsConfirmed = {}, closedDays = [], groupSlots = [], toast, onSendSMS, goto, onOpenSettings }) {
  ...
  useEffect(() => {
    ...
    const lastSeen = (typeof localStorage !== 'undefined') 
      ? localStorage.getItem('sosun_finance_alert_seen') : null;
    if (lastSeen === todayStr) return; // 오늘 이미 봤으면 스킵
    
    const alerts = getMonthlyAlerts(todayDate);
    const urgent = alerts.filter(a => {
      if (a.urgent) return true; // 종소세, 부가세 등은 항상 urgent
      if (a.dDay) {
        const num = parseInt(a.dDay.replace(/[^0-9]/g, '')) || 0;
        return a.dDay === 'D-day' || num <= 7; // D-day 또는 7일 이내
      }
      return false;
    });
    
    if (urgent.length > 0) {
      setFinanceAlertItems(urgent);
  ...
  const dismissFinanceAlert = () => {
    ...
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem('sosun_finance_alert_seen', toYMD(new Date()));
    }
  };
  ...
  const capacityStatus = useMemo(() => {
    ...
    (groupSlots || []).filter(time => !SKIP_CAPACITY_TIMES.includes(time)).forEach(time => {
      ...
      result.push({ 
        ...
        tueCount: tueMembers.length, 
        thuCount: thuMembers.length 
      });
    });
    return result;
  }, [members, groupSlots]);
  ...
  return (
    <div className="px-3 pb-28 pt-2 space-y-3">
      ...
      {capacityStatus.length > 0 && (
        <div className="rounded-2xl p-3.5" style={{ backgroundColor: theme.card, border: `1px solid ${theme.line}` }}>
          <div className="flex items-center gap-1.5 mb-3">
            ...
            <span className="text-[10px] ml-auto italic" style={{ color: theme.inkMute, fontFamily: theme.serif }}>
              탭하면 회원 보기 →
            </span>
          </div>
          <div className="grid items-center gap-2" style={{ gridTemplateColumns: '52px 1fr 1fr' }}>
            ...
            {capacityStatus.map(({ time, tueCount, thuCount }) => {
              const renderCell = (count) => {
                ...
                return (
                  <div className="rounded-lg px-2 py-1.5 text-center relative cursor-pointer transition-transform active:scale-[0.97]" 
                    onClick={() => setCapacitySheetTime(time)}
              ...
              return (
                <React.Fragment key={time}>
                  <div 
                    onClick={() => setCapacitySheetTime(time)}
                    className="text-[13px] font-semibold text-right pr-1 cursor-pointer" 
                    style={{ color: theme.ink }}>{time}</div>
                  {renderCell(tueCount)}
                  {renderCell(thuCount)}
                </React.Fragment>
              );
      ...
      {/* 📊 정원현황 시트 (시간대별 회원 목록) */}
      {capacitySheetTime !== null && (
        <CapacitySheet
          time={capacitySheetTime}
          allTimes={(groupSlots || []).filter(t => capacityStatus.find(c => c.time === t))}
          capacityStatus={capacityStatus}
          capacity={CAPACITY}
          onClose={() => setCapacitySheetTime(null)}
          onChangeTime={(t) => setCapacitySheetTime(t)}
        />
      )}

      {/* 💼 사업 일정 팝업 (홈 진입 시 자동) */}
      {showFinanceAlert && financeAlertItems.length > 0 && (
        <Modal open={true} onClose={dismissFinanceAlert} 
          title="💼 사업 일정 알림" 
          sub={`${financeAlertItems.length}건의 중요 일정이 있어요`}>
          <div className="space-y-2 mb-3">
            {financeAlertItems.map((a, i) => (
              <div key={i} className="rounded-xl p-3 flex gap-2.5"
                style={{
                  backgroundColor: a.urgent ? 'rgba(184,84,80,0.08)' : 'rgba(212,181,116,0.1)',
                  border: a.urgent ? '1px solid rgba(184,84,80,0.3)' : '1px solid rgba(212,181,116,0.4)',
                }}>
                <div className="flex-shrink-0 rounded-lg flex items-center justify-center"
                  style={{
                    width: 36, height: 36,
                    backgroundColor: a.urgent ? 'rgba(184,84,80,0.15)' : '#FFE8C9',
                    fontSize: 16,
                  }}>
                  {a.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-[13px] font-bold" style={{ color: theme.ink }}>
                    {a.title}
                    {a.dDay && <span className="ml-1.5 text-[11px]" style={{ color: a.urgent ? '#B85450' : '#C26B4A' }}>· {a.dDay}</span>}
                  </div>
                  {a.desc && <div className="text-[11px] mt-0.5" style={{ color: theme.inkSoft }}>{a.desc}</div>}
                  <div className="text-[10.5px] mt-1 font-semibold" style={{ color: a.urgent ? '#B85450' : '#8A6418' }}>
                    ⏰ {a.deadline}
                  </div>
                </div>
              </div>
          ...
          <div className="text-[10px] text-center mb-2 italic" style={{ color: theme.inkMute }}>
            오늘 이 알림은 다시 안 보여요. 통계 탭에서 자세히 확인하세요.
          </div>
          <Button onClick={dismissFinanceAlert} className="w-full">확인했어요</Button>
        </Modal>
      )}
    ...
    <Modal open={true} onClose={onClose} title={titleText} maxWidth="max-w-md">
      ...
      {recapFor && onSendSMS && (
        <RecapComposerModal
          ...
          onSend={async (bodyStatus, lastClass, keyPoints, direction) => {
            // ⭐ 문자 보내는 걸로 끝나지 않고, 회원 경과(progressLog)에도 같이 저장
            if (setMembers) {
              const newEntry = {
                id: `recap_${toYMD(new Date())}_${Date.now()}`,
                date: date,
                classType: '개인레슨 관찰 정리',
                bodyState: (bodyStatus || []).filter(Boolean).join('\n'),
                afterChange: (lastClass || []).filter(Boolean).join('\n'),
                memo: [
                  keyPoints?.length ? `[기억할 내용]\n${keyPoints.filter(Boolean).join('\n')}` : '',
                  direction ? `[앞으로 방향]\n${direction}` : '',
                ].filter(Boolean).join('\n\n'),
                autoGenerated: false,
              };
              const updatedMembers = members.map(m =>
                m.id === recapFor.id ? { ...m, progressLog: [...(m.progressLog || []), newEntry] } : m
              );
              setMembers(updatedMembers);
              await saveKey(K.members, updatedMembers);
            }
...
function DayLogEditor({ date, entries, sessions, onClose, onSave, groupSlots = [] }) {
  ...
  const dayTimes = useMemo(() => {
    ...
    Object.keys(sessions || {}).forEach(k => {
      if (k.startsWith(date + '_')) {
        const t = k.split('_')[1];
        if (t) set.add(t);
      }
    });
    // 이미 기록된 시간도 포함 (옛 기록 수정 가능하게)
    (entries || []).forEach(e => { if (e.time) set.add(e.time); });
    // 그래도 비어있으면 TIME_PRESETS 전체로 폴백
    if (set.size === 0) return TIME_PRESETS;
    return [...set].sort();
  }, [date, sessions, entries]);

  const [list, setList] = useState(entries);
  const [time, setTime] = useState(dayTimes[0] || '11:00');
...
function FinanceCard({ targetMonth, revenue, netRevenue }) {
  ...
  return (
    <div className="rounded-2xl p-3.5 mb-3" 
      style={{ 
        background: 'linear-gradient(135deg, #FAF7EE 0%, #F5EBC8 100%)',
        border: '1px solid #D4B574',
      }}>
      {/* 헤더 */}
      <div className="flex items-center gap-1.5 mb-3">
        <span style={{ fontSize: 16 }}>💼</span>
        <span className="font-bold text-[14px]" style={{ color: '#2D2A26', fontFamily: theme.serif }}>
          {targetMonth.getMonth() + 1}월의 사업 체크
        </span>
      </div>
      
      {/* 알림 박스들 */}
      <div className="space-y-2 mb-3">
        {monthlyAlerts.map((a, i) => (
          <div key={i} className="rounded-xl p-2.5 flex gap-2.5"
            style={{
              backgroundColor: a.urgent ? 'rgba(184,84,80,0.08)' : 'white',
              border: a.urgent ? '1px solid rgba(184,84,80,0.3)' : '1px solid rgba(212,181,116,0.4)',
            }}>
            <div className="flex-shrink-0 rounded-lg flex items-center justify-center"
              style={{
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[12px] font-bold" style={{ color: '#2D2A26' }}>
                {a.title}
                {a.dDay && <span className="ml-1 text-[10px]" style={{ color: a.urgent ? '#B85450' : '#C26B4A' }}>· {a.dDay}</span>}
              </div>
              {a.desc && <div className="text-[10.5px] mt-0.5" style={{ color: '#6B7568' }}>{a.desc}</div>}
              <div className="text-[10px] mt-0.5 font-semibold" style={{ color: a.urgent ? '#B85450' : '#8A6418' }}>
                ⏰ {a.deadline}
              </div>
            </div>
          </div>
      ...
      {/* 매출 기반 추천 */}
      {revenue > 0 && (
        <div className="rounded-xl p-3 mb-3" style={{ backgroundColor: 'rgba(255,255,255,0.5)' }}>
          <div className="text-[10px] font-bold mb-2" style={{ color: '#6B5410', letterSpacing: '0.05em' }}>
            💰 이번 달 매출 기반 추천
          </div>
          <div className="space-y-1.5 text-[11.5px]">
            <div className="flex justify-between items-baseline pb-1.5" style={{ borderBottom: '1px dashed rgba(212,181,116,0.3)' }}>
              <span style={{ color: '#6B7568' }}>이번 달 매출</span>
              <span className="font-semibold tabular-nums" style={{ color: '#2D2A26' }}>{revenue.toLocaleString()}원</span>
            </div>
            <div className="flex justify-between items-baseline pb-1.5" style={{ borderBottom: '1px dashed rgba(212,181,116,0.3)' }}>
              <span style={{ color: '#6B7568' }}>세금용 저축 (매출 5%)</span>
              <span className="font-semibold tabular-nums" style={{ color: '#B85450' }}>{taxRecommended.toLocaleString()}원</span>
            </div>
            <div className="flex justify-between items-baseline">
              <span style={{ color: '#6B7568' }}>실수령 (수수료 차감 후)</span>
              <span className="font-semibold tabular-nums" style={{ color: '#5C8A5C' }}>{netRevenue.toLocaleString()}원</span>
            </div>
          </div>
        </div>
      )}
...
// === 재무 대시보드 헬퍼 ===
// 월별 사업 일정 알림 (간이과세자 기준)
function getMonthlyAlerts(targetMonth) {
  ...
  const OPEN_MONTH = 3;
  const OPEN_DAY = 17;
  // 첫 종소세 신고 연도 (개업한 다음 해 5월)
  const FIRST_TAX_YEAR = OPEN_YEAR + 1;
  
  // 매달 공통: 월세, 4대보험
  alerts.push({
    icon: '🏠',
    title: '월세·관리비',
    desc: '월세 450,000원 · 관리비 약 80,000원',
    deadline: `매월 22일 결제`,
    day: 22,
    urgent: false,
  });
  ...
  alerts.push({
    icon: '💊',
    title: '건강보험·국민연금',
    desc: '지역가입자 자동납부 (소득 적으면 감면 신청 가능)',
    deadline: '매월 10일 자동납부',
    day: 10,
    urgent: false,
  });
  
  // 5월 종소세 - 첫 신고 연도부터 표시
  if (month === 5 && year >= FIRST_TAX_YEAR) {
    alerts.unshift({
      icon: '📋',
      title: '종합소득세 신고',
      desc: '전년도 사업 소득 신고 + 납부',
      deadline: '5월 31일까지 · 홈택스 또는 세무사',
      day: 31,
      urgent: true,
    });
  }
  ...
  // 1월 부가세 신고 (간이과세자)
  if (month === 1 && year > OPEN_YEAR) {
    alerts.unshift({
      icon: '📊',
      title: '부가가치세 신고 (간이과세자)',
      desc: '전년도 1년치 매출 신고 (연 4,800만원 미만은 면제)',
      deadline: '1월 25일까지 · 홈택스',
      day: 25,
      urgent: true,
    });
  }
  ...
  // 항상 표시되는 추천 사항 (urgent 아님)
  alerts.push({
    icon: '💰',
    title: '노란우산공제 추천',
    desc: '매월 적립 → 소득공제 + 퇴직금. 소상공인진흥공단에서 가입',
    deadline: '신생 사업자 강추',
    day: 0,
    urgent: false,
    note: true,
  ...
  // D-day 계산
  const isCurrentMonth = today.getFullYear() === targetMonth.getFullYear() && 
                        today.getMonth() === targetMonth.getMonth();
  if (isCurrentMonth) {
    alerts.forEach(a => {
      if (a.day === 0) return; // 0 = 상시
      const dDay = a.day - today.getDate();
      if (dDay >= 0 && dDay <= 30) {
        a.dDay = dDay === 0 ? 'D-day' : `D-${dDay}`;
      }
    });
  }
...
function StatsView({ members, trials, sessions, closedDays = [], feeRates }) {
  ...
  const classStats = useMemo(() => {
    ...
    const trialByName = {};
    (trials || []).forEach(t => {
      if (t.name) trialByName[t.name] = t;
    });
  ...
  }, [sessions, members, trials, targetYM, monthOffset, targetMonth]);
  ...
  const dailyStats = useMemo(() => {
    ...
    Object.values(sessions).forEach(s => {
      ...
      attendees.forEach(p => {
        ...
        // ⭐ 스타터 패키지 개인레슨(price=0으로 묶여있음)은 회당 70,000원 고정으로 계산
        if (!perSession && pass.bundleOf && pass.category === 'private') {
          perSession = 70000;
        }
        ...
        return (
          <div className="rounded-2xl p-3" style={{ backgroundColor: '#F5EBC8', border: '1px solid #C9A961' }}>
            ...
            {bonusMembers.length === 0 ? (
              <div className="text-[11px]" style={{ color: '#9A7A40' }}>
                이번달 수강권 수정에서 보너스를 소급 적용하면 여기 표시돼요
              </div>
            ) : (
              <div className="space-y-1.5">
              ...
              <>
                ...
                {timeSlotStats.map(t => (
                  <div key={t.time} className="mb-2.5">
                    <div className="flex justify-between text-[11px] mb-1 flex-wrap gap-1">
                    ...
                    <div className="flex justify-between items-center mb-1">
                      <div className="h-1.5 rounded-full overflow-hidden flex-1" style={{ backgroundColor: theme.cardAlt }}>
                        <div style={{ width: `${(t.count / timeSlotStats[0].count) * 100}%`, height: '100%', backgroundColor: theme.accent }} />
                      </div>
                      {t.totalRevenue > 0 && (
                        <span className="ml-2 text-[11px] font-bold tabular-nums" style={{ color: theme.accent, fontFamily: theme.serif }}>
                ...
                {/* 총 합계 */}
                {(() => {
                  const totalCount = timeSlotStats.reduce((s, t) => s + t.count, 0);
...
function PassEditModal({ pass, onClose, onSave }) {
  ...
  const [rhythmBonus, setRhythmBonus] = useState(pass.rhythmBonus || 0);
  const [reviewEventBonus, setReviewEventBonus] = useState(pass.reviewEventBonus || 0);
  const [mayEventBonus, setMayEventBonus] = useState(pass.mayEventBonus || 0);
  ...
  const handleSave = () => {
    ...
    const bonusFields = {};
    if (Number(rhythmBonus) > 0) bonusFields.rhythmBonus = Number(rhythmBonus);
    if (Number(reviewEventBonus) > 0) bonusFields.reviewEventBonus = Number(reviewEventBonus);
    if (Number(mayEventBonus) > 0) bonusFields.mayEventBonus = Number(mayEventBonus);
    onSave({
  ...
  return (
    <Modal open={true} onClose={onClose} title="수강권 수정">
      <div className="space-y-2.5">
        ...
        {/* 보너스 소급 적용 */}
        <div className="rounded-xl p-3 space-y-2" style={{ backgroundColor: '#F5EBC8', border: '1px solid #C9A961' }}>
          <div className="text-[11px] font-bold" style={{ color: '#6B5410' }}>🎁 보너스 소급 적용 (선택)</div>
          {[
            { label: '리듬 완주 보너스', val: rhythmBonus, set: setRhythmBonus },
            { label: '리뷰 이벤트 보너스', val: reviewEventBonus, set: setReviewEventBonus },
            { label: '5월 이벤트 보너스', val: mayEventBonus, set: setMayEventBonus },
          ].map(({ label, val, set }) => (
            <div key={label} className="flex items-center justify-between">
              <span className="text-[11px]" style={{ color: '#7A5C00' }}>{label}</span>
              <div className="flex items-center gap-2">
                <button onClick={() => set(Math.max(0, Number(val) - 1))}
                  className="w-7 h-7 rounded-full text-sm font-bold flex items-center justify-center"
                  style={{ backgroundColor: '#FFF8E8', color: '#7A5C00' }}>−</button>
                <span className="text-[13px] font-bold w-5 text-center" style={{ color: '#3A2A06' }}>+{val}</span>
                <button onClick={() => set(Number(val) + 1)}
                  className="w-7 h-7 rounded-full text-sm font-bold flex items-center justify-center"
                  style={{ backgroundColor: '#FFF8E8', color: '#7A5C00' }}>+</button>
              </div>
            </div>
    ...
    <Modal open={true} onClose={onClose} title="리듬 수련 출석 현황">
      <div className="space-y-3">
        ...
        <div>
          ...
          {weekKeys.map((wk, i) => {
            const isExtraWeek = weekMap[wk]['화']?.status === 'extra' || weekMap[wk]['목']?.status === 'extra';
            const prevIsNotExtra = i > 0 && weekMap[weekKeys[i-1]]['화']?.status !== 'extra' && weekMap[weekKeys[i-1]]['목']?.status !== 'extra';
            const showDivider = isExtraWeek && prevIsNotExtra;
            return (
              <React.Fragment key={wk}>
                {showDivider && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, marginTop: 2 }}>
                    <div style={{ flex: 1, height: 1, backgroundColor: theme.line }} />
                    <span style={{ fontSize: 9, color: theme.inkMute, whiteSpace: 'nowrap' }}>도전 기간 이후</span>
                    <div style={{ flex: 1, height: 1, backgroundColor: theme.line }} />
                  </div>
                )}
