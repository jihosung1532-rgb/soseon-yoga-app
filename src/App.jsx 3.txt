          ...
          <table className="w-full text-[12px]" style={{ minWidth: 600 }}>
            <thead>
              <tr>
                ...
                {days.map((d, i) => {
                  const isHol = HOLIDAYS.has(toYMD(d));
                  return (
                  <th key={i} className="py-2 px-1 font-medium"
                    style={{
                      color: isHol ? theme.danger : theme.ink,
                      borderBottom: `1px solid ${theme.line}`,
                      borderLeft: `1px solid ${theme.line}`,
                      backgroundColor: isHol ? theme.dangerBg + '40' : smallGroupDay(d) ? theme.accentSoft : theme.cardAlt,
                    }}>
                    <div style={{ fontWeight: 700 }}>{WEEK_KR[d.getDay()]}</div>
                    <div className="text-[11px] font-normal" style={{ color: isHol ? theme.danger : theme.inkSoft }}>
              ...
              {TIME_PRESETS.map((time) => (
                <tr key={time}>
                  <td className="py-2 px-2 font-bold tabular-nums align-top"
                    style={{ color: theme.accent2, backgroundColor: theme.cardAlt + '88', borderBottom: `1px solid ${theme.line}`, fontFamily: theme.serif, fontSize: 13 }}>
                    {time}
                  </td>
                  {days.map((d, i) => {
                    ...
                    return (
                      <td key={i}
                        onClick={() => !isHol && setSlotModal({ date: d, time, existing: sessions[`${toYMD(d)}_${time}`], autoFill: view?.isAuto ? view.participants : null })}
                        className="align-top p-1 transition-colors"
                        style={{
                          borderBottom: `1px solid ${theme.line}`,
                          borderLeft: `1px solid ${theme.line}`,
                          backgroundColor: isHol ? theme.dangerBg + '40' : isToday ? theme.highlight : smDay ? theme.accentSoft + '55' : 'transparent',
...
export default function App() {
  ...
  // 앱 시작 시 세션 복원 — 로그인 우회 (Supabase 연동 전)
  useEffect(() => {
    (async () => {
      try {
        sbAuth.token = 'bypass';
  ...
  // 로그인 화면 — Supabase 연동 후 활성화 (if !authed)
  if (false) {
    return (
